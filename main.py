...
def deduplicate_close_punches(punch_list, threshold_minutes=3):
    """
    Checks if consecutive punches happen within `threshold_minutes` (e.g. 3 minutes).
    ...
    for p in punch_list:
        ...
        # If tapped within 3 minutes (180 seconds) of previous tap
...
def categorize_punches(raw_punch_list, is_saturday):
    ...
    Directional window categorization:
      - Clock In: Earliest punch in morning window
      - Break Out: Latest punch in early lunch window (< 13:15)
      - Break In: Earliest punch in late lunch window (>= 13:15)
      - Clock Out: Latest punch in evening window
    """
    ...
    if is_saturday:
        # Saturday: Morning In (< 11:30) and Afternoon Out (>= 11:30)
        morning = [
            p
            for p in p_tuples
        ...
        # 1. Clock In -> EARLIEST morning punch
        if morning:
            c_in = morning[0][1]
            assigned_punches.append(c_in)

        # 2. Lunch Punches (11:15 to 15:30)
        # Split lunch around 13:15: Break Out (< 13:15) and Break In (>= 13:15)
        ...
        # Break Out -> LATEST punch before/around lunch start
        if lunch_out_candidates:
            b_out = lunch_out_candidates[-1][1]
            assigned_punches.append(b_out)

        # Break In -> EARLIEST punch returning from lunch (e.g. takes 14:09 instead of 14:18)
        if lunch_in_candidates:
            b_in = lunch_in_candidates[0][1]
            assigned_punches.append(b_in)

        # Fallback if both punches fell into the same sub-window
        if b_out == "--" and b_in == "--":
            all_lunch = [
                p
                for p in p_tuples
        ...
        # 3. Clock Out -> LATEST evening punch
        if evening:
            c_out = evening[-1][1]
            assigned_punches.append(c_out)

    # Route leftovers (e.g. the 14:18 re-punch) and double-taps to Multiple Punch columns
    extra_punches = [
...
def process_time_card(
    ...
    # Fast lookup for specific person or company-wide special days
    special_lookup = {}
    ...
    # Build calendar dates excluding Sundays
    calendar_dates = []
    curr = min_d
    while curr <= max_d:
        if curr.weekday() != 6:
    ...
    for emp_id, meta in sorted(emp_meta.items()):
        ...
        for w_date in calendar_dates:
            ...
            # Check individual person match first, then company-wide 'ALL'
            special_type = special_lookup.get((date_str, name)) or special_lookup.get(
                (date_str, "ALL")
            )

            raw_times_str = raw_punches.get((emp_id, date_str))

            # SCENARIO 1: NO PUNCH RECORD
            ...
            if early_remark:
                if (
                    status in ["Normal", "Saturday (Half Day)"]
                    or "off day" in status
                    or "Leave" in status
                    or "Holiday" in status
                ):
...
def build_excel_workbook(df):
    ...
    for row_idx, row_vals in enumerate(overview_data, start=2):
        for col_idx, val in enumerate(row_vals, start=1):
            ...
            # Highlight absences in light coral
            if col_idx == 5 and val > 0:
                cell.fill = absent_fill
                cell.font = font_bold
            # Highlight accumulated late columns in amber
            elif col_idx in [8, 9, 10, 11] and str(val) != "0:00":
                cell.fill = late_fill
                cell.font = font_bold
            # Highlight punch issues count in warm orange
    ...
    for (emp_id, emp_name), emp_group in df.groupby(
        ...
        for r_offset, (_, row_data) in enumerate(emp_group.iterrows()):
            ...
            for col_idx, val in enumerate(row_vals, 1):
                ...
                # Priority 1: Full Row Coloring for Absent (Coral Red)
                if is_absent:
                    cell.fill = absent_fill
                    if col_idx == 12:
                        cell.font = font_bold

                # Priority 2: Full Row Coloring for Off Days / Holidays / Leaves (Soft Grey)
                elif is_offday:
                    cell.fill = offday_fill
                    if col_idx == 12:
                        cell.font = font_bold

                # Priority 3: Working Days with specific cell exceptions
                else:
                    # Late to Work, Lunch Late, Total Late, Early Leave (Amber Yellow)
                    if col_idx in [7, 8, 9, 10] and val != "--":
                        cell.fill = late_fill
                        cell.font = font_bold

                    # Punch Irregularities (Warm Orange)
                    if col_idx == 12 and (
...
def api_process():
    ...
    try:
        ...
        CURRENT_DF, detected_start, detected_end = process_time_card(
